...
Structures
  ...
  If two structures S_{1} and S_{2} have the same class and the value
  of each slot in S_{1} is the same under ‘equalp’ as the value of
  the corresponding slot in S_{2}.

Hash Tables

  ‘equalp’ descends hash tables by first comparing the count of
  entries and the ‘:test’ function; if those are the same, it
